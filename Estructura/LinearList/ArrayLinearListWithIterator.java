...
public class ArrayLinearListWithIterator <Item> implements LinearList <Item>{
	protected int size;
	...
	public ArrayLinearListWithIterator(int newSize){
		this.size = 0;
	...
	protected class IteratorForArrayList implements Iterator<Item>{
		private int nextIndex;
		private boolean wasNextCalled;
		
		protected IteratorForArrayList(){
			this.nextIndex = 0;
			this.wasNextCalled = false;
		}
		
		public boolean hasNext() {
			return(this.nextIndex != ArrayLinearListWithIterator.this.size-1);
		}
		...
		public void remove(){
			if(this.wasNextCalled){
				this.nextIndex--;
				ArrayLinearListWithIterator.this.remove(nextIndex);
			}
			else{
